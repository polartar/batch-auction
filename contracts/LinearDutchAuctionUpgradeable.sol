...
abstract contract LinearDutchAuctionUpgradeable is OwnableUpgradeable {
    /**
    ...
    function __LinearDutchAuctionUpgradeable_init(DutchAuctionConfig memory config, uint256 expectedReserve) internal onlyInitializing {
        __Ownable_init();
    ...
    function cost(uint256 n) public view returns (uint256) {
        ...
        return n * (cfg.startPrice - MathUpgradeable.min((block.timestamp - cfg.startPoint) / cfg.decreaseInterval, cfg.numDecreases) * cfg.decreaseSize);
    }
}