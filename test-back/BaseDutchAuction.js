...
const BaseDutchAuction = require("../artifacts/contracts/BaseDutchAuction.sol/BaseDutchAuction.json");
...
const HASH_PREFIX_DISCOUNTED = "Discounted Verification:";

before(async function () {
  this.Base = contract.fromABI(BaseDutchAuction.abi, BaseDutchAuction.bytecode);
...
it("cannot mint while sale is inactive and can mint while sale is active", async function () {
  ...
  await this.base.setAuctionStartPoint(secondsSinceEpoch, { from: owner });
...
it("only owner can enable sale state", async function () {
  await expectRevert(
    this.base.setAuctionStartPoint(secondsSinceEpoch, { from: other }),
    "Ownable: caller is not the owner"
  );
});
...
it("whitelist discounted mint allows whitelisted users with proper price", async function () {
  ...
  await this.base.mintWhitelistDiscounted(hash, ownerSignature, 1, {
...
it("whitelist discounted mint allows whitelisted users with improper price", async function () {
  ...
  await expectRevert(
    this.base.mintWhitelistDiscounted(hash, ownerSignature, 1, {
  ...
  await expectRevert(
    this.base.mintWhitelistDiscounted(wrongHash, wrongOwnerSignature, 1, {
      value: BigNumber.from(PRICE_DISCOUNTED),
      from: other,
    }),
    "The address hash does not match the signed hash."
